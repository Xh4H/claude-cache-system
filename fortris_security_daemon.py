...
import psutil
from fortris_security_cache import FortrisSecurityCache, FileType
...
logger = logging.getLogger("FortrisDaemon")

class FortrisSecurityDaemon:
    """High-performance daemon for Fortris security cache operations"""
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = 19848):
        ...
        self.handlers = {
            ...
            'fortris_info': self._handle_fortris_info
        }
        
        logger.info("Fortris Security Daemon initialized")
    
    def start(self):
        """Start the Fortris daemon server"""
        try:
            ...
            logger.info(f"🔒 Fortris Security Daemon started on {self.host}:{self.port}")
            logger.info("Fortris is ready for enterprise security analysis")
            ...
            while self.running:
                try:
                    ...
                    if self.running:
                        logger.error("Socket error in Fortris accept loop")
                        
        except Exception as e:
            logger.error(f"Failed to start Fortris daemon: {e}")
            sys.exit(1)
        ...
        try:
            ...
            try:
                ...
                if command in self.handlers:
                    ...
                    response = {'error': f'Unknown Fortris command: {command}'}
                ...
                self.stats['errors'] += 1
                logger.error(f"Fortris error handling command: {e}")
            ...
            logger.error(f"Fortris client handling error: {e}")
        finally:
            client_socket.close()
    ...
    def _handle_cache(self, file_path: str, force: bool = False) -> Dict[str, Any]:
        ...
        if entry:
            ...
            return {
                ...
                'cached': False,
                'file': file_path,
                'error': 'Failed to cache file with Fortris'
            }
    ...
    def _handle_stats(self) -> Dict[str, Any]:
        ...
        metrics.update({
            ...
            'daemon_type': 'Fortris Security Daemon v3.0'
        })
        
        return metrics
    
    def _handle_security_report(self) -> Dict[str, Any]:
        """Handle Fortris security report command"""
    ...
    def _handle_git_update(self, base_ref: str = "HEAD~1", target_ref: str = "HEAD") -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'status': 'success', 
                'message': 'Fortris git update completed',
    ...
    def _handle_scan(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'status': 'success', 
                'message': 'Fortris vulnerability scan completed',
                'fortris_analysis': True
            }
        except Exception as e:
            return {'status': 'error', 'error': str(e)}
    ...
    def _handle_health(self) -> Dict[str, Any]:
        """Handle Fortris health check command"""
        try:
            # Check cache system
            metrics = self.cache.get_performance_metrics()
            ...
            health_status = {
                ...
                'fortris_security_analysis': True
            }
    ...
    def _handle_set_repo(self, repo_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'status': 'success', 
                'message': f'Fortris git repo set to: {repo_path}',
                'fortris_git_integration': True
            }
        except Exception as e:
            return {'status': 'error', 'error': str(e)}
    ...
    def _handle_vulnerabilities(self, severity: Optional[str] = None) -> Dict[str, Any]:
        ...
        with self.cache._get_db_connection() as conn:
            if severity:
                cursor = conn.execute(
                    'SELECT * FROM fortris_vulnerabilities WHERE severity = ? AND resolved = 0',
                    (severity,)
                )
            else:
                cursor = conn.execute(
                    'SELECT * FROM fortris_vulnerabilities WHERE resolved = 0'
                )
        ...
        return {
            ...
            'analyzed_by': 'Fortris Security Analyzer'
        }
        ...
        try:
            ...
            for partition in self.cache.partitions.values():
                with self.cache._get_db_connection(partition.partition_id) as conn:
                    conn.execute('DELETE FROM fortris_partition_cache')
                    conn.commit()
            
            # Clear main index
            with self.cache._get_db_connection() as conn:
                conn.execute('DELETE FROM fortris_cache')
                conn.execute('DELETE FROM fortris_vulnerabilities')
                conn.execute('DELETE FROM fortris_git_commits')
                conn.commit()
            ...
            return {
                'status': 'success', 
                'message': 'Fortris cache cleared',
                'fortris_cleared': True
            }
    ...
    def _handle_optimize(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'status': 'success', 
                'message': 'Fortris optimization completed',
                'fortris_optimized': True
            }
            
        except Exception as e:
            return {'status': 'error', 'error': str(e)}
    
    def _handle_fortris_info(self) -> Dict[str, Any]:
        """Handle Fortris info command"""
        return {
            'name': 'Fortris Security Cache System',
            'version': '3.0',
    ...
    def _monitor_loop(self):
        ...
        while self.running:
            try:
                ...
                time.sleep(60)
                metrics = self.cache.get_performance_metrics()
                logger.info(f"Fortris metrics: Hit rate: {metrics['hit_rate_percent']:.1f}%, "
                ...
                logger.error(f"Fortris monitor loop error: {e}")
    
    def stop(self):
        """Stop the Fortris daemon"""
        self.running = False
        ...
        logger.info("Fortris Security Daemon stopped")


class FortrisDaemonClient:
    """Client for communicating with the Fortris cache daemon"""
    ...
    def send_command(self, command: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        """Send command to Fortris daemon"""
        try:
            ...
            return {'error': f'Failed to communicate with Fortris daemon: {e}'}


def main():
    """Main entry point for Fortris daemon"""
    parser = argparse.ArgumentParser(description='Fortris Security Cache Daemon v3.0')
    ...
    # Client commands
    subparsers = parser.add_subparsers(dest='command', help='Fortris Commands')
    
    # Cache command
    cache_parser = subparsers.add_parser('cache', help='Cache a file with Fortris')
    ...
    # Stats command
    subparsers.add_parser('stats', help='Show Fortris cache statistics')
    
    # Security report command
    subparsers.add_parser('security-report', help='Generate Fortris security report')
    ...
    # Scan command
    subparsers.add_parser('scan', help='Run Fortris vulnerability scan')
    
    # Health command
    subparsers.add_parser('health', help='Check Fortris daemon health')
    
    # Set repo command
    repo_parser = subparsers.add_parser('set-repo', help='Set git repository')
    repo_parser.add_argument('path', help='Repository path')
    
    # Fortris info command
    subparsers.add_parser('info', help='Show Fortris system information')
    ...
    if args.daemon:
        # Run as daemon
        daemon = FortrisSecurityDaemon(args.host, args.port)
        
        # Handle signals
        def signal_handler(signum, frame):
            logger.info("Received signal, shutting down Fortris daemon...")
            daemon.stop()
            sys.exit(0)
        ...
        # Run as client
        client = FortrisDaemonClient(args.host, args.port)
        ...
        if args.command == 'cache':
            ...
            result = client.send_command('fortris_info')
        else:
            result = {'error': f'Unknown Fortris command: {args.command}'}
        
        print(json.dumps(result, indent=2))
        
    else:
        print("🔒 Fortris Security Cache System v3.0")
